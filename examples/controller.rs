...
use hailstorm::grpc::{
    self, AgentGroup, AgentMessage, AgentUpdate, ClientDistribution, CommandItem,
    ControllerCommand, LaunchCommand, LoadSimCommand,
};
...
async fn initialize_agents(
    ...
    sender
        .send(ControllerCommand {
            ...
            commands: vec![
                CommandItem {
                    command: Some(load_command.clone()),
                },
                CommandItem {
...
async fn handle_messages(
    mut msg_stream: Streaming<AgentMessage>,
    sender: Sender<ControllerCommand>,
) {
    ...
    while let Some(msg_res) = msg_stream.next().await {
        match msg_res {
            Ok(msg) => {
                ...
                registered_agents.retain(|_idx, update| {
                    update
                        .update_ts()
                ...
                let summary: HashMap<String, HashMap<u32, u32>> =
                    registered_agents
                        .iter()
                        .fold(HashMap::new(), |mut acc, (_, upd)| {
                            for model_stats in &upd.stats {
                                let model_acc = acc
                                    .entry(model_stats.model.clone())
                                    .or_insert_with(HashMap::new);
                                for state_stats in
                                    model_stats.states.iter().flat_map(|s| s.states.iter())
                                {
                                    let acc_state_stats =
                                        model_acc.entry(state_stats.state_id).or_insert(0);
                    ...
                    sender
                        .send(ControllerCommand {
                            ...
                            commands: vec![CommandItem {
                                command: Some(Command::UpdateAgentsCount(
                                    registered_agents.len() as u32
                                )),
                            }],
                        })
                        .await
                        .expect("Error sending UpdateAgentsCount")
                }
            }
            Err(err) => {
...
async fn main() {
    ...
    let config: HailstormAgentConfig =
        compose_config("config/hailstorm-controller").expect("Error loading config");
